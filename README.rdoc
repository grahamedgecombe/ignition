...
link:https://github.com/grahamedgecombe/ignition/raw/master/doc/logo.png
...
initializer (e.g. <tt>config/initializers/ignition.rb</tt>) or an environment
configuration file (e.g. <tt>config/environments/production.rb</tt>).
...
You can configure caching using the <tt>caching_method</tt> setting.

<tt>:page</tt> uses the Rails page caching. <tt>:page_without_layout</tt> uses
action caching without the caching your application's layout. You can also set
it to <tt>:none</tt> for no caching to be performed (this is the default).

== Tips

If Ignition cannot find a page, it will set the <tt>X-Cascade</tt> header to
<tt>pass</tt> and set the response status to 404. This will cause the Rails
